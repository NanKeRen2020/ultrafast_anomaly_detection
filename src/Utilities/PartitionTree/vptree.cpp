...
VPTree::VPTree(PatchManager& pm_, const TreeParams& paramTree) : nbSelectionVP(paramTree.nbVP), subSampling(paramTree.subS)
{
	param = paramTree.mprms;
...
void VPTree::retrieveFromTree(std::vector<std::tuple<float, unsigned, float>>& localResult, const unsigned pidx)
{
	Node* currentNode = this->root; 
	// Find the leaf in which the patch resides
	while(currentNode->child1 != NULL && currentNode->child2 != NULL)
	...
	for(unsigned i = currentNode->left, j = 0; i <= currentNode->right; ++i,++j)
	{
		float dist = pm->distance(pidx, idxPatches[i], raw_dist);
		localResult.push_back(std::make_tuple(dist, idxPatches[i], raw_dist));
	}
}
