...
class LocalRefinement : public LocalPatchPropagation {
	public:
		 ...
		 */
		LocalRefinement(PatchManager& pm_, ForestManager& fm_, int kNN_, const ModelParams& prms) { pm = &pm_; fm = &fm_; kNN = kNN_; params = prms;};
		...
		int estimateSimilarPatches(std::vector<std::tuple<float, unsigned, float>> &index, const unsigned pidx, bool excludeYourself = false);
		/**
		 * @}
		 */
	private:
